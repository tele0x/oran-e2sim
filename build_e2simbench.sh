#!/usr/bin/env bash
##
## Container build for O-RAN E2 KPM Simulator for Bouncer xApp (RIC Benchmarking)
##
...
container=$(buildah --name e2sim_bench from registry.access.redhat.com/ubi8/ubi)

buildah config --label maintainer="Federico 'tele' Rossi <ferossi@redhat.com>" $container

# Install packages to compile e2sim
buildah run $container dnf install -y --nobest --skip-broken --nogpgcheck --disableplugin=subscription-manager g++ gcc-c++ gcc rpm-build cmake make git lksctp-tools autoconf automake libtool ncurses-devel \
...
buildah run $container git clone "https://gerrit.o-ran-sc.org/r/it/test.git" /oran

# Compile e2sim
buildah config --workingdir /oran/ric_benchmarking/e2-interface/e2sim $container
buildah run $container mkdir build
buildah config --workingdir /oran/ric_benchmarking/e2-interface/e2sim/build $container
...
# Build kpm_sim
buildah config --workingdir /oran/ric_benchmarking/e2-interface/e2sim/e2sm_examples/kpm_e2sm/ $container
buildah run $container rm -fr .build
buildah run $container mkdir .build
buildah config --workingdir /oran/ric_benchmarking/e2-interface/e2sim/e2sm_examples/kpm_e2sm/.build $container
buildah run $container cmake ..
buildah run $container make install

# Commit to local container storage
buildah commit $container e2sim_bench
